...
$id=$_GET["id"];
$sql=$conexion->query(" SELECT * FROM emergencia WHERE id=$id ");
...
<body>
    <form class="col-4 p-3 m-auto" method="POST">
        <h3 class="text-center text-secondary">Modificar emergencia</h3>
        <input type="hidden" name="id" value="<?= $_GET["id"] ?>">
        <?php
        include "controlador/mod_emergencia.php";
        while ($datos=$sql->fetch_object()) { ?>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="turno" value="<?= $datos->turno ?>">
            </div>
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Recepcionista</label>
                <input type="text" class="form-control" name="recepcionista" value="<?= $datos->recepcionista ?>">
            </div>
            <div class="mb-3">
